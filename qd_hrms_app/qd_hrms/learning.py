...
def process_certification_expiry_notifications():
	...
	for cert in frappe.get_all(
		...
		filters={"status": ("not in", ("Revoked", "Renewed"))},
		fields=[
			"name",
			"employee",
			"employee_name",
		...
		if -60 <= days <= 90 and not cert.renewal_request:
			request_name = _open_certification_renewal_request(cert, days)
			if request_name:
				updates["renewal_request"] = request_name
		frappe.db.set_value("QD Training Certification", cert.name, updates, update_modified=False)


def _open_certification_renewal_request(cert, days):
	if not frappe.db.exists("DocType", "QD Training Request"):
		return None
	existing = frappe.db.exists(
		"QD Training Request",
		{
			...
			"course": cert.course,
			"approval_status": ["not in", ("Rejected", "Withdrawn", "Cancelled")],
			"docstatus": ["<", 2],
		},
	)
	if existing:
	...
	request = frappe.get_doc(
		{
			"doctype": "QD Training Request",
			"employee": cert.employee,
			"course": cert.course,
			"preferred_start_date": today(),
			"reason": _("Auto-opened to renew certification {0} expiring on {1}.").format(
				cert.name, cert.expiry_date
			),
			"business_benefit": _("Keep mandatory certification current."),
		}
	)
...
def supersede_previous_certificates(certificate):
	"""Mark earlier certificates for the same employee and course as renewed."""
	if not certificate or not certificate.employee or not certificate.course:
		return
	...
	if not previous:
		return
	latest = previous[0]
	if certificate.meta.has_field("renewed_from") and not certificate.renewed_from:
		frappe.db.set_value(
			"QD Training Certification",
			certificate.name,
			"renewed_from",
	...
	for name in previous:
		...
		if frappe.get_meta("QD Training Certification").has_field("renewed_to"):
			updates["renewed_to"] = certificate.name
		frappe.db.set_value("QD Training Certification", name, updates, update_modified=False)
