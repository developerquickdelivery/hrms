...
def execute(filters=None):
	...
	if frappe.db.exists("DocType", "QD Employee License"):
		seen = set()
		license_filters = {"status": ["in", ["Due for Renewal", "Renewal In Progress", "Expired"]]}
		if filters.get("employee"):
			license_filters["employee"] = filters.get("employee")
		if filters.get("department"):
			license_filters["department"] = filters.get("department")
		if filters.get("company"):
			license_filters["company"] = filters.get("company")
		for row in frappe.get_all(
			"QD Employee License",
			fields=[
				"name",
			...
			seen.add(row.name)
			data.append({
				"item": f"License: {row.license_type}",
				"reference": row.name,
		...
		upcoming_filters = {
			...
			"status": ["not in", ["Renewed", "Revoked"]],
		}
		if filters.get("employee"):
			upcoming_filters["employee"] = filters.get("employee")
		if filters.get("department"):
			upcoming_filters["department"] = filters.get("department")
		if filters.get("company"):
			upcoming_filters["company"] = filters.get("company")
		for row in frappe.get_all(
			"QD Employee License",
			fields=[
				"name",
