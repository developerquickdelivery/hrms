...
@frappe.whitelist()
def licenses_due_30d():
	if not frappe.db.exists("DocType", "QD Employee License"):
		return _card(0)
	frappe.has_permission("QD Employee License", throw=True)
	end = add_days(today(), 30)
	due = frappe.db.count(
		"QD Employee License",
		{"status": ["in", ["Due for Renewal", "Renewal In Progress", "Expired"]]},
	)
	upcoming = frappe.db.count(
		"QD Employee License",
		{
			"expiry_date": ["between", [today(), end]],
			"status": ["not in", ["Renewed", "Revoked", "Due for Renewal", "Renewal In Progress", "Expired"]],
		},
	)
	return _card(due + upcoming)
