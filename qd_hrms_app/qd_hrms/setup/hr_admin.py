...
def ensure_workspace():
	...
	for section_label, entries in SECTIONS:
		...
		if not available:
			continue
		available_sections.append(section_label)
		doc.append(
			"links",
			{
				"type": "Card Break",
...
def _available_sections():
	sections = []
	for section_label, entries in SECTIONS:
		if any(frappe.db.exists("DocType", doctype) for _label, doctype in entries):
...
def verify():
	...
	missing_sections = [label for label in available_sections if label not in links]
	if missing_sections:
